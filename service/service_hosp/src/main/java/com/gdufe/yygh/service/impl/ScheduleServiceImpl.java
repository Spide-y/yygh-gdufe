...
import com.gdufe.yygh.model.hosp.BookingRule;
...
import com.gdufe.yygh.repository.ScheduleRepository;
...
import com.gdufe.yygh.vo.hosp.BookingScheduleRuleVo;
...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    public Schedule getScheduleById(String scheduleId) {
        Schedule schedule = repository.findById(scheduleId).get();
    ...
    public Map<String, Object> getBookingScheduleRule(int page, int limit, String hoscode, String depcode) {
        ...
        BookingRule rule = hospital.getBookingRule();

        //获取可预约的日期的数据(分页)
        IPage pages = this.getListDate(page,limit,rule);
        List<Date> dateList = pages.getRecords();

        //获取可预约日期里面科室的剩余预约数
        Criteria criteria = Criteria.where("hoscode").is(hoscode).and("depcode").is(depcode).and("workDate").in(dateList);
        Aggregation agg = Aggregation.newAggregation(//将数据按workDate进行分组，然后求出每个工作日的医生数docCount和可预约数availableNumber和reservedNumber
        ...
        AggregationResults<BookingScheduleRuleVo> aggregate = template.aggregate(agg, Schedule.class, BookingScheduleRuleVo.class);
        ...
        for (int i=0,len=dateList.size();i<len;i++){//如果在预约周期中有可预约的workDate 放入bookingScheduleRuleVoList中
            ...
            if (i==len-1 && page == pages.getPages()){
            ...
            //如果当天过了停号时间，不能预约
            if (i == 0 && page == 1){
                DateTime stopTime = this.getDateTime(new Date(),rule.getStopTime());
        ...
        //放号时间
        baseMap.put("releaseTime", rule.getReleaseTime());
        //停号时间
        baseMap.put("stopTime", rule.getStopTime());
    ...
    public Map<String, Object> getScheduleRule(int page, int limit, String hoscode, String depcode) {
        ...
        //根据医院编号 和 科室编号查询科室信息
        Criteria criteria = Criteria.where("hoscode").is(hoscode).and("depcode").is(depcode);//构造查询条件
        ...
        Aggregation agg = Aggregation.newAggregation(
                ...
                .first("workDate").as("workDate")//设置别名
                .count().as("docCount")
                .sum("reservedNumber").as("reservedNumber")//统计号源数量
                .sum("availableNumber").as("availableNumber"),//统计号源数量
                Aggregation.sort(Sort.Direction.DESC,"workDate"),
                Aggregation.skip((page-1)*limit),//分页
                Aggregation.limit(limit)
        );
        //调用方法，执行Aggregation
        AggregationResults<BookingScheduleRuleVo> aggResult = template.aggregate(agg, Schedule.class, BookingScheduleRuleVo.class);
        ...
        AggregationResults<BookingScheduleRuleVo> totalAggResult = template.aggregate(totalAgg,Schedule.class, BookingScheduleRuleVo.class);
    ...
    public void remove(String hoscode, String hosScheduleId) {
        //根据医院编号 和 科室编号查询科室信息
        Schedule schedule = repository.getScheduleByHoscodeAndHosScheduleId(hoscode,hosScheduleId);
    ...
    public Page<Schedule> findPage(int page, int limit, ScheduleQueryVo scheduleQueryVo) {
        ...
        Page<Schedule> schedules = repository.findAll(example, pageable);
        return schedules;
    }
    ...
    public void save(Map<String, Object> map) {
        ...
        Schedule scExist = repository.getScheduleByHoscodeAndHosScheduleId(schedule.getHoscode(),schedule.getHosScheduleId());

        if (scExist!=null){
            scExist.setUpdateTime(new Date());
            scExist.setIsDeleted(0);
            scExist.setStatus(1);
            repository.save(scExist);
        }else {
    ...
    private IPage getListDate(int page,int limit,BookingRule bookingRule){
        ...
        //如果可以显示的数据小于7，直接显示 否则分页
        if (end > dateList.size()) {//小于
            end = dateList.size();
        }
